...
настройкой, а не правкой кода: в `.env` есть локальный прокси, а по умолчанию сервер идёт
напрямую. Третий маршрут, релей на Cloudflare Worker, развёрнут и проверен вживую - и оказался
тупиковым: OpenRouter отклоняет запросы с периферии Cloudflare, поэтому из РФ рабочим обходом
остаётся VPN. Замеры в разделе «Если openrouter.ai недоступен».
...
Дальше: `npm run verify` прогоняет всё - линт, типы релея, 130 тестов и обе сборки.
...
Способов дойти до апстрима три. Выбирается в `.env`, писать код не нужно. Рабочих из РФ -
два, и третий туда не попал не по недосмотру: замеры ниже.
...
**Релей.** Третий способ задумывался как решение без VPN вообще: маленький Cloudflare Worker в
`worker/` перекладывает исходящие запросы в OpenRouter, и ни разработчику, ни тому, кто открывает
демо, VPN не нужен.
...
Порядок именно такой. `secret put` принимает **только имя** секрета позиционным аргументом, а
значение спрашивает интерактивно; передать его вторым аргументом - `Unknown argument`. До первого
деплоя команда падает на «Worker not found».
...
**Релей развёрнут, работает как прокси и не решает задачу.** Это главный результат раздела, и он
отрицательный. Релей поднят на живом аккаунте, проверен и оставлен в репозитории как рабочий
обратный прокси, но дойти через него до OpenRouter нельзя.

Сначала то, что работает. Проба `GET /` отвечает без секрета, а запрос без заголовка или с чужим
секретом получает отказ от самого Worker:

```text
GET  /                     -> 200 {"ok":true,"service":"openrouter-relay"}
GET  /models, чужой токен  -> 403 {"error":"Bad or missing relay token."}
```

Дальше начинается блокировка. Любой запрос, который Worker честно пересылает в OpenRouter,
возвращается так:

```text
GET /key, верный токен -> 403 за 71 мс
...
`set-cookie` с доменом `openrouter.ai` и `permissions-policy` со Stripe - подписи самого
OpenRouter. Значит запрос до OpenRouter доехал, и отказал уже он. Отказ приходит за 2-70 мс, то
есть срабатывает правило на периферии, до приложения: **ключ ещё никто не смотрел**. Так и есть -
настоящий ключ из `.env` получает ровно тот же `403`, что и заведомо поддельный. Отказ зональный:
не только `/api/v1/*`, но и корень `openrouter.ai` отдаёт полноценную страницу
`Attention Required! | Cloudflare`.

Дальше я перебирал то, что вообще можно перебрать со стороны Worker:

| Что менялось | Результат |
| --- | --- |
| заведомо поддельный ключ / настоящий ключ из `.env` | `403` одинаково |
| `User-Agent` по умолчанию / браузерный Chrome / `curl` | `403` одинаково |
| полный браузерный отпечаток: `sec-fetch-*`, `sec-ch-ua`, `Origin`, `Referer`, `Accept-Language` | `403` |
| пустой запрос без ключа вовсе | `403` |

Отдельно проверено, что дело не в Cloudflare вообще. Временный диагностический Worker в той же
периферии, тем же запросом, в одном запуске:
...
`example.com` живёт за Cloudflare и отвечает `200`. Значит запрет не «Cloudflare к Cloudflare», а
конкретно политика OpenRouter против запросов с периферии Cloudflare. Причина видна в эхе
заголовков: каждый исходящий подзапрос Worker несёт `Cf-Worker: <аккаунт>.workers.dev`, который
рантайм дописывает сам, и уходит по IPv6 из диапазона Cloudflare (`2a06:98c0::/29`). Попытка
выставить `Cf-Worker: ''` ничего не меняет - рантайм проставляет своё значение поверх.

**Что из этого следует.** Релей на Cloudflare для этой задачи тупиковый: смена исходящего адреса
и есть его единственная функция, а именно она и не срабатывает. То же ограничение распространяется
на любую площадку внутри сети Cloudflare, а не только на `workers.dev`. Не проверено (и не буду
утверждать): релей на хосте вне Cloudflare - VPS, другой serverless-провайдер. Судя по тому, что
`example.com` проходит, дело не в самом факте датацентра, а в ASN Cloudflare, так что шанс есть;
проверять это на своей машине я не могу.

**Что остаётся человеку из РФ.** Ровно то, что было до релея: VPN через `OUTBOUND_PROXY`, либо
прямой путь, если он вне РФ. Проверяющему задание в России это стоит знать заранее - снаружи
блокировка выглядит как «чат сломан», а не как «провайдер закрыл доступ».
...
подождать минуту там, где ждать нечего. Разбор в журнале, пункт 33.

По умолчанию в репозитории `OPENROUTER_BASE_URL` пуст: сервер идёт напрямую, а релей включается
двумя строками - но включать его незачем, пока он не переехал на хост вне Cloudflare.
...
- Три маршрута до OpenRouter: напрямую, через локальный прокси и через релей на Cloudflare
  Worker. Переключается в `.env`, код при этом не меняется. Из РФ работают первые два: релей
  развёрнут и как прокси исправен, но OpenRouter его исходящий адрес отклоняет.
...
- Релей на Cloudflare не работает, и это проверено, а не предположено. Он поднят на живом
  аккаунте, отвечает на пробу и отказывает без секрета, но любой пересланный запрос получает
  `403 Access denied by security policy` от самого OpenRouter: настоящий ключ и поддельный, любой
  `User-Agent`, полный браузерный отпечаток - всё одинаково. Контроль в том же запуске:
  `example.com`, тоже за Cloudflare, отвечает `200`. Причина - исходящий адрес: рантайм
  дописывает к каждому подзапросу `Cf-Worker: <аккаунт>.workers.dev` и уходит по IPv6 из
  диапазона Cloudflare. Перебить это из воркера нельзя. Значит релей нужен на хосте вне
  Cloudflare - не проверено, но, судя по `example.com`, дело в ASN, а не в самом факте датацентра.
- Релей не обходит и лимиты: он не меняет ключ и не балансирует запросы, поэтому суточную квоту
  ему обойти нечем. Даже если он заработает на другом хосте, он решает доступ, а не квоту.
- Когда апстрим недоступен целиком (например, из РФ без обхода), каталог моделей остаётся пустым.
  Причина пустого каталога теперь запоминается и доезжает до пользователя своим кодом
  (`UPSTREAM_BLOCKED`), а не общей отговоркой «свободных исполнителей нет».
- Проверка прокси на старте делается один раз. Если VPN-клиент живёт и слушает порт, а туннель
  уже упал, сервер продолжает считать прокси рабочим, и каждый запрос падает с `fetch failed` -
  ровно тот же симптом «приложение сломалось», от которого эта проверка и появилась. Найдено
  вживую: `proxy: true` в `/api/health` и `lastError: fetch failed` рядом. Лечится повторной
  проверкой при сетевой ошибке - это не сделано.
...
- **Тесты на клиенте.** Сейчас покрыт сервер и релей: 124 теста сервера (парсер SSE, разбор
  ошибок апстрима, валидация, отбор бесплатных моделей, порядок кандидатов, пул прощупывания,
  ротация ключей, память реестра о причине пустого каталога, персональный промпт, диалоговая
  политика) и 6 тестов релея против поддельного апстрима. На клиенте стоит добавить хотя бы
  компонентные тесты и Playwright-сценарий на «стоп -> часть ответа сохранилась».
...
- **Повторная проверка прокси при сетевой ошибке.** Сейчас порт проверяется один раз на старте, и
  упавший посреди сессии туннель оставляет сервер с `proxy: true` и `fetch failed` на каждом
  запросе. Одного повтора без прокси после сетевой ошибки хватило бы, чтобы это лечилось само.
...
33. **Отказ провайдера выглядел как нехватка моделей, и из-за этого мёртвый ключ не отходил.**
    Заметил пользователь: «при нерабочем ключе должен сразу подхватываться рабочий». Пошёл
    проверять и нашёл не одну ошибку, а связку из двух.

    Первая: пустой каталог имел один ответ на все случаи - «свободных исполнителей сейчас нет».
    Это правда про каталог, который честно вернулся пустым, и неправда про провайдера, который
    отказал целиком. Отказ приходил как `403 Access denied by security policy`, то есть по статусу
    выглядел как проблема ключа, а сообщение советовало подождать минуту.

    Вторая, и она дороже: пустой каталог отдавался кодом `NO_MODELS`, который ротация ключей
    **не считает ключевой ошибкой**. Чтение каталога стояло снаружи цикла ротации, поэтому
    отклонённый ключ валил обнаружение, каталог оставался пустым, запрос отвечал `NO_MODELS`,
    и на этом всё заканчивалось - ключ не переключался никогда. Ровно то, на что жаловался
    пользователь, и по коду незаметно: каждая часть по отдельности выглядела разумно.
    ...
    ротации, чтобы отклонённый ключ переключался на месте. И появился код `UPSTREAM_BLOCKED` для
    403, который не является конвертом OpenRouter: различие строится на форме тела, а не на
    статусе, потому что статус у «ключ отклонён» и «адрес не пускают» один и тот же, а лечение
    разное. Блок намеренно **не** считается ключевой ошибкой - он выдан адресу, и вторая попытка
    с другим ключом стоит пользователю лишнего круга.

    Заодно всплыло третье: на любой ключевой ошибке модель уходила в остывание. То есть ротация
    от мёртвого ключа парковала здоровые модели - механизм чинил одно и ломал другое. Теперь
    модель остывает только тогда, когда виновата она.

    Проверено вживую на заблокированном маршруте: `/api/chat` отвечает `502 UPSTREAM_BLOCKED` с
    причиной в тексте, а не `503 NO_MODELS`. Пять тестов на различение двух видов 403 и на память
    реестра, включая забывание причины после успешного круга.

34. **Релей на Cloudflare оказался тупиком, и выяснилось это только на живом аккаунте.**
    Развернул его как задумано: `wrangler login`, `wrangler deploy`, секрет. Всё зелёное - проба
    отвечает, без секрета отказ, тесты проходят. А до OpenRouter запрос не доходит.

    Первое объяснение, которое я записал в README, было неверным. Я рассуждал так: локальный релей
    с российской машины получает `403`, значит развёрнутый с периферии Cloudflare его обойдёт -
    ровно это и стояло в предыдущей редакции раздела. Замер показал обратное: с периферии тот же
    `403`. Пришлось переписывать раздел на противоположный по смыслу.

    От неверного вывода спас контрольный опыт. Запрос через тот же Worker к `example.com`, который
    тоже живёт за Cloudflare, вернул `200`. Значит дело не в «Cloudflare против Cloudflare» и не в
    самом релее, а в политике OpenRouter конкретно.

    Дальше гипотезы перебирались по одной и всегда с контролем: поддельный ключ против настоящего,
    три `User-Agent` (включая браузерный Chrome), полный браузерный отпечаток с `sec-fetch-*`,
    `sec-ch-ua`, `Origin` и `Referer`. Ничего не сдвинулось, и это уже само по себе ответ:
    решение принимается не по запросу, а по адресу. Эхо заголовков показало, по какому признаку -
    `Cf-Worker: <аккаунт>.workers.dev`, который рантайм дописывает сам, плюс IPv6 из диапазона
    Cloudflare. Попытка выставить `Cf-Worker` пустым не помогла: рантайм проставляет своё значение
    поверх. Ещё один диагностический Worker для этих замеров был развёрнут и удалён.

    Итог: код релея остался, шесть его тестов остались, а раздел README переписан наоборот -
    теперь он объясняет, почему этот путь не работает. Это единственное место в проекте, где я не
    починил код, а признал, что решение неверное.
...
Ошибка 33 - про **композицию**, и это отдельная категория. Ни одна её половина не выглядела
ошибкой: «пустой каталог значит нет моделей» разумно, «на `AUTH` переключаем ключ» разумно,
«читаем каталог один раз перед циклом» разумно. Неверным было только их соединение, а увидеть
это можно было единственным способом - проследить путь отклонённого ключа до конца, вместо того
чтобы проверять каждый шаг отдельно. Правило, которое забираю: когда что-то не работает,
проходить цепочку целиком, а не осматривать её звенья.
...
24. `docs` - раздел про недоступность OpenRouter из РФ, обновлённые скриншоты, записи 29-32 в
    ИИ-логе.
25. `fix(server)` - причина пустого каталога запоминается и отдаётся своим кодом, обнаружение
    переехало внутрь цикла ротации ключей, `UPSTREAM_BLOCKED` отделён от `AUTH`, модель больше не
    остывает из-за чужого ключа.
