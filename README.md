...
`npm run verify` прогоняет всё: линт, типы релея, 135 тестов и обе сборки.
...
| --- | --- | --- |
| Через VPN | `OUTBOUND_PROXY` - включён в `.env.example` | локальная разработка, когда клиент VPN поднят |
| Напрямую | `OUTBOUND_PROXY=` (пусто) | машина вне РФ или с открытым доступом |
| Через релей | `OPENROUTER_BASE_URL` + `OPENROUTER_RELAY_TOKEN` | не работает: OpenRouter отклоняет запросы с периферии Cloudflare |

**VPN.** `OUTBOUND_PROXY` стоит в `.env.example` **включённым**, а не закомментированным. Причина
простая: машина, на которой это будут запускать, скорее всего не достанет OpenRouter напрямую, и
человек, копирующий пример в `.env`, не должен догадываться об этом по коду `403`. Порт - это
локальный HTTP-порт клиента VPN: 10809 у v2rayN, 7890 у clash, 2080 у sing-box.

```text
OUTBOUND_PROXY=http://127.0.0.1:10809
```

Ничего не предполагается. Порт проверяется один раз на старте, TCP-коннектом с таймаутом в
полторы секунды, и если никто не слушает, сервер пишет об этом в лог и идёт напрямую:
...
**Чужой текст не проходит наружу как есть.** Тело ответа апстрима цитируется в сообщении об
ошибке - так ошибка объясняет себя сама, и это удобно, пока не вспомнить, что писал тело
кто-то другой. OpenRouter заголовок `Authorization` в теле не возвращает, но это его сегодняшнее
поведение, а не гарантия, а браузер - единственное место, где ключ не должен читаться: в Network
он в теле ответа виден ровно так же, как в заголовке запроса. Поэтому текст ошибки проходит через
`redactSecrets` перед тем, как стать сообщением: типизированные ошибки чистятся в конструкторе
`AppError` (значит, чистыми оказываются и JSON-ответ, и событие `error` в SSE, и сохранённый
`lastError` в `/api/health`, и лог сервера), а текст ошибки внутри потока - на входе в дельту, где
он и попадает в SSE. Фильтр ищет `sk-` с длинным хвостом, поэтому обычное слово вроде `task-force`
он не задевает - на это есть тест.
...
и трактуется как список из одного ключа. Обе переменные читаются **одним и тем же разбором**,
поэтому список через запятую принимает и `OPENROUTER_API_KEY`: вставить второй ключ в переменную,
где уже лежит ключ, слишком естественно, чтобы наказывать за это ответом `401 User not found`.
...
**Пустой экран** держит поле ввода **внутри** себя: вопрос человека и есть содержание экрана, а поле
закрывает блок. На широком экране между подсказкой и полем стоят примеры вопросов - они читаются
как меню «что тут можно спросить», а поле закрывает список. На телефоне примеров нет: четыре
белые карточки с той же рамкой и скруглением, что и у поля, стоят прямо над ним, и экран читается
как пять полей, четыре из которых ничего не делают при промахе. Без примеров блок короткий, и поле
поднимается под подсказку, а весь блок центрируется.

Центрирование сделано автоотступами (`margin-block: auto`), а не `justify-content: center`.
Автоотступы забирают свободное место, когда оно есть, и схлопываются в ноль, когда его нет, -
на экране ниже блока заголовок не уезжает за верхнюю границу, чего `center` не гарантирует.
Как только появляется первая реплика, поле уезжает вниз, в привычное место чат-инпута.
...
**Высота поля** не считается в JavaScript. `field-sizing: content` в CSS просит браузер подогнать
высоту под текст, `max-height` задаёт потолок, дальше поле скроллится внутри себя. Раньше здесь
стоял `useLayoutEffect`, который один раз за монтирование читал `scrollHeight` и записывал его
обратно инлайн-стилем, - и любая причина, по которой это единственное чтение оказывалось неверным,
превращалась в поле, застрявшее на максимуме до перезагрузки страницы. Число, которое браузер
пересчитывает из содержимого каждый кадр, застрять так не может. Там, где `field-sizing` пока не
поддерживается (Safari, Firefox), поле остаётся высотой в одну строку `rows={1}` и скроллит текст
внутри себя: писать многострочное по-прежнему можно, следует за текстом не рамка.

**На телефоне поле 16px, а не 15px, как весь остальной текст.** Это не вкус, а порог: iOS Safari
зумит страницу, когда фокус получает поле с размером шрифта меньше 16px, и обратно не раззумивает.
Страница остаётся увеличенной и сдвинутой, шапка уходит за верхний край, и вернуть её можно только
перезагрузкой.
...
- **Тесты на клиенте.** Сейчас покрыт сервер и релей: 129 тестов сервера (парсер SSE, разбор ошибок
...
20. **Иконки на iPhone схлопывались в один пиксель, и Chrome этого не показывал.** Lucide рисует
    иконки инлайновым SVG и задаёт размер атрибутами `width`/`height` - обычными числами, без
    единиц. Внутри HTML эти атрибуты читаются как CSS-объявления, а голое число длиной не является,
    поэтому объявление отбрасывается. Chrome к этому снисходителен, WebKit - нет: иконка остаётся
    вообще без использованного размера, а поскольку она лежит во флексе, алгоритм сжимает её до
    нуля. Пользователь увидел это как «вместо иконки темы цветной кружок с точкой в центре».

    Первая правка была селектором по атрибуту - `svg[width='16'][height='16']`. В харнессе на
    WebKit она отработала, а на живом телефоне нет: размер иконки там остался `1.0x1.0`. Вывод
    оказался не про конкретный селектор, а про способ: **размер задаёт контейнер** (`.icon-button >
    svg`), как у единственной иконки, которая не ломалась ни разу, - телеграмной, у которой размер
    всегда приходил из класса, а не из атрибута. Плюс `flex: none`, иначе иконка без размера
    сжимается в ничто. Мораль: «проверено в WebKit» и «проверено на телефоне» - два разных
    утверждения, и подменять второе первым нельзя.

21. **Два ключа, вставленные в переменную для одного ключа, выглядели как два негодных ключа.**
    `OPENROUTER_API_KEY=sk-…, sk-…` читалось целиком - с запятой и пробелом - и уходило одним
    bearer-токеном. OpenRouter отвечал `401 User not found`, то есть текст ошибки указывал на ключи,
    а негодной была форма значения: оба ключа по отдельности отвечали `200`. Починил тем, что обе
    переменные проходят через один разбор и список через запятую принимает любая из них; тест
    добавлен с пробелом после запятой, потому что именно так это и печатают.

22. **Свежая копия репозитория не работала из коробки, и это была не ошибка в коде, а ошибка
    решения.** `.env` в репозиторий не попадает (в нём ключ), поэтому человек, скачавший проект,
    копирует `.env.example`. В примере `OUTBOUND_PROXY` стоял **закомментированным** - как опция
    для тех, у кого уже есть VPN. На машине в РФ это означало: сервер идёт напрямую, получает от
    Cloudflare `403` и отвечает «сервис отклонил запрос с этого адреса». Диагностика в коде была
    верной и прямо называла причину, но появлялась только после отказа, а нужный шаг оставался
    ручным - то есть человек должен был прочитать сообщение, найти строку в файле и перезапустить
    сервер. Проверяющий, который просто копирует пример и запускает `npm run dev`, до этого места
    не доходил.

    Правильно было с самого начала поставить значение **включённым**. Порт всё равно проверяется
    TCP-коннектом на старте, поэтому на машине без клиента VPN сервер одной строкой сообщает, что
    прокси не отвечает, и идёт напрямую - то есть безопасный откат уже был, а выключенное по
    умолчанию значение ничего не защищало. Разница между «опция выключена» и «опция включена с
    безопасным откатом» - ровно один шаг для того, кто запускает, и он оказался решающим.

23. **Поле ввода открывалось высотой в максимум, и воспроизвести это не удавалось ни в одном
    браузере.** Симптом: пустое поле на телефоне открывалось на всю высоту, которую ему разрешал
    `max-height`, и лечилось только перезагрузкой страницы. Причина по коду: высота записывалась
    инлайн один раз за монтирование, из одного чтения `scrollHeight`. Если это чтение в момент
    монтирования оказывалось неверным - контейнер успел растянуть поле, расширение браузера тронуло
    элемент, шрифт приехал после чтения, - неверное число оставалось инлайн-стилем до перезагрузки,
    потому что перезаписывать его было нечем: эффект висел на изменении значения, а значение не
    менялось.

    Что именно испортило чтение в конкретном Chrome, выяснить не удалось. И Chromium, и WebKit, и
    обе версии кода - до правки и после - давали ровно 41px при любом размере окна, включая
    специально растянутый родитель и шпион на геттере `scrollHeight`. Это и есть вывод: число,
    записанное один раз из одного замера, нельзя ни воспроизвести по требованию, ни защитить
    проверкой - от него можно только избавиться. Высота теперь приходит из CSS
    (`field-sizing: content`, `max-height` как потолок), инлайн-стиля нет вообще, и застревать
    нечему.

    Побочно выяснилось, что `field-sizing` меряет и плейсхолдер: на 320px строка «Опишите Амалии
    задачу...» переносилась и тянула за собой поле на две строки. Лечится `white-space: nowrap` на
    `::placeholder` - поле остаётся высотой в строку, а текст подсказки обрезается многоточием.

24. **iOS Safari зумил страницу при фокусе в поле и обратно не возвращал.** Симптом со скриншота:
    после начала набора шапка уехала вверх и вправо и там осталась. Причина не в вёрстке: Safari
    зумит страницу при фокусе на поле, у которого `font-size` меньше 16px, а поле наследовало
    `--text-base: 15px` от `body`. Поднять размер до 16px ниже брейкпоинта оказалось достаточно.

    Отдельно стоит отметить, что этот дефект видел только владелец телефона. Ни эмуляция
    устройства в Chrome DevTools, ни WebKit в Playwright зум при фокусе не воспроизводят: он
    привязан к реальному вводу с экранной клавиатурой, а не к ширине вьюпорта. Это второй случай в
    этом логе, когда «не воспроизводится» означало «не тот инструмент», а не «дефекта нет».
...
Ветка `feat/amalia-chat` влита в `main` через Pull Request с описанием решений -
[#1](https://github.com/CityStas/saldo/pull/1): это требование задания, и по слоям ревью читается
